...
const EditProfile = () => {
  ...
  const handleCertificationUpdate = (certType, field, value) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const validateFile = (file, certType) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setFileErrors((prev) => ({
        ...prev,
        [certType]: translatedText['Only PNG, JPG, and PDF files are allowed'],
      }));
      return false;
    }
    ...
    if (file.size > maxSize) {
      setFileErrors((prev) => ({
        ...prev,
        [certType]: translatedText['File size must be less than 5MB'],
      }));
      return false;
    }

    setFileErrors((prev) => ({ ...prev, [certType]: null }));
    return true;
  };

  const handleFileUpload = async (e, certType) => {
    const file = e.target.files[0];

    if (!file || !validateFile(file, certType)) {
      return;
    }
  ...
  const handleAddCertification = (certType) => {
    setFormData((prev) => ({
      ...prev,
      certifications: [
        ...prev.certifications,
  ...
  const handleDeleteCertification = (certType) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const prepareFormData = () => {
    ...
    formData.certifications.forEach((cert) => {
      if (cert.document) {
        form.append('certifications', cert.document);
      }
    });
      ...
      <div className={styles.container}>
        ...
        <div className={styles.certifications}>
          <div className={styles.certTitle}>
            {translatedText['Certifications']}
          </div>
          <div className={styles.certContainer}>
            {certificationOptions.map((certOption, index) => {
              const userCert = formData.certifications?.find(
                (cert) => cert.type === certOption.value
              );

              return (
                <div className={styles.boxShadow} key={index}>
                  <h3>{certOption.label}</h3>
                  <img
                    className={styles.profileImg}
                    src={certOption.image}
                    alt={certOption.label}
                  />
                  {userCert ? (
                    <div className={styles.certDetails}>
                      {/* Existing certification inputs */}
                      <div>
                        <label>{translatedText['Expiry Date']}:</label>
                        <input
                          type='date'
                          value={userCert.expiryDate.split('T')[0]}
                          onChange={(e) =>
                      ...
                      <div>
                        <label>{translatedText['Reference Number']}:</label>
                        <input
                          type='text'
                          value={userCert.referenceNumber}
                          onChange={(e) =>
                            handleCertificationUpdate(
                              certOption.value,
                              'referenceNumber',
                      ...
                      <div className={styles.fileUploadContainer}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleFileUpload(e, certOption.value)
                          }
                        />
                        {fileErrors[certOption.value] && (
                          <div className={styles.errorMessage}>
                            {fileErrors[certOption.value]}
                          </div>
                        )}
                        {userCert.documentName && (
                      ...
                      <button
                        onClick={() =>
                          handleDeleteCertification(certOption.value)
                        }
                        className={styles.deleteBtn}
                        type='button'
                      >
                        {translatedText['Delete Certificate']}
                      </button>
                    </div>
                  ) : (
                    <button
                      onClick={() => handleAddCertification(certOption.value)}
                      className={styles.addCertButton}
                    >
                      {translatedText['Add Certificate']}
                    </button>
                  )}
                </div>
              );
