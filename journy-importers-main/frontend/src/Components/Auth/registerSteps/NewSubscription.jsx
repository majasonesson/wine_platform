...
import { signIn, updateRole } from '../../../Services/Httpclient';
...
export const NewSubscription = () => {
  ...
  useEffect(() => {
    ...
    const role = localStorage.getItem('signupRole'); // Get stored role

    const subscription = async () => {
      if (email && password) {
        // First update the role
        if (role) await updateRole(email, role);
        ...
        localStorage.removeItem('signupRole'); // Clean up role storage
  ...
  return (
    <div className={styles.modal}>
      <div className={styles.modalContent}>
        <h2>{translatedText['Payment Successful!']}</h2>
      </div>
    </div>
  );
};
