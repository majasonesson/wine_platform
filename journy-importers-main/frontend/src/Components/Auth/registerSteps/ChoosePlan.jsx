...
import { createRegisterSubscription } from '../../../Services/stripe';
...
const ChoosePlan = ({ formData }) => {
  const [loading, setLoading] = useState(false);
  const [translatedText, setTranslatedText] = useState({});
  const [language, setLanguage] = useState(
    localStorage.getItem('language') || ''
  );
  const [readyFormData, setReadyFormData] = useState(null);
  ...
  useEffect(() => {
    if (formData) {
      const data = {
        ...
        certificationDetails: formData.get('certificationDetails'),
      };

      setReadyFormData(data);
    }
  }, [formData]);
  ...
  const handleSubscription = async (priceId) => {
    try {
      if (!readyFormData) {
        showModal(translatedText['Please complete the form first']);
        return;
      }
      setLoading(true);

      const response = await createRegisterSubscription(priceId, readyFormData);
      if (!response.ok) {
  ...
  return (
    <Elements stripe={stripePromise}>
      <>
        ...
        {/* Logo and Back button are here but code is commented out */}
        ...
        <div className={styles.pricesSection}>
          <h2 className={styles.planTitle}>{translatedText['Choose plan']}</h2>
