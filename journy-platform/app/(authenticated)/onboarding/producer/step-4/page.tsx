...
export default function ProducerStep4() {
  ...
  // 1. Hämta producentens profil-ID
  useEffect(() => {
    async function getProfile() {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        const { data } = await supabase
          .from('producer') // Kopplat till producer-tabellen
          .select('id')
          .eq('user_id', user.id)
          .single();
  ...
  // 2. Spara data och skicka till dashboard
  const handleFinish = async () => {
    if (!profileId) return;
    setLoading(true);
    const { error } = await supabase
      .from('producer') // Uppdaterar producer-tabellen
      .update({
    ...
    if (!error) {
      router.push('/dashboard'); // Skickar producenten till huvud-dashboarden
    } else {
      setLoading(false);
...
// Identisk Stepper-komponent med 1% steg
