...
export const generateFullDescription = (
    ...
    if (characteristics.length === 0 && texture.length === 0 && aromas.length === 0) return "";

    // 1. Combine Characteristics and Texture (Adjectives)
    const combinedBase = [...characteristics, ...texture];
    let firstPart = "";

    if (combinedBase.length > 0) {
        // Lowercase all but potentially use comma/and structure
        const lowercased = combinedBase.map(s => s.toLowerCase());
        if (lowercased.length > 1) {
            firstPart = `${lowercased.slice(0, -1).join(', ')} and ${lowercased.slice(-1)[0]}`;
        } else {
            firstPart = lowercased[0];
        }
        // Capitalize the very first letter of the sentence
        firstPart = firstPart.charAt(0).toUpperCase() + firstPart.slice(1);
    }

    // 2. Build the subject (Wine)
    let description = firstPart ? `${firstPart} wine` : "Wine";

    // 3. Add Aromas if present
    if (aromas.length > 0) {
        const aromaPart = aromas.length > 1
            ? `${aromas.slice(0, -1).join(', ').toLowerCase()} and ${aromas.slice(-1)[0].toLowerCase()}`
            : aromas[0].toLowerCase();

        description += ` with balanced notes of ${aromaPart}.`;
    } else {
        description += ".";
    }
